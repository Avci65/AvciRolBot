...
ROLE_EMOJIS = {
    "tavcı": "💂", "tarikat avcisi": "💂", "yancı": "💋", "hood": "💋",
    "melek": "👼", "koruyucu melek": "👼", "detective": "🕵️","dete": "🕵️",
    "gözcü": "👳‍♀️", "otacı": "🍃", "muhtar": "🎖", "silah": "🔫",
    "prens": "👑", "prenses": "👑", "çiftçi": "👨‍🌾", "aldeanisimo": "👨‍🌾",
    "barışcıl": "☮️", "demirci": "⚒", "çığırtkan": "📰", "pregonero": "📰",
    "uyutucu": "💤", "şifacı": "🌟", "korsan": "🏴‍☠️", "gözcü çırağı": "🙇", 
    "apps": "🙇", "kahin": "🌀", "tilki": "🦊", "avcı": "🎯",
    "yaşlı bilge": "👵🏻","yb": "👵🏻", "sarhoş": "🍻", "mason": "👷", 
    "masontek": "👷", "seyirci": "👁", "sgy": "👁","sgv": "👁👳‍♀️",
    "hayalet": "ghost", "şaşı": "👀", "bizca": "👀", "uyurgezer": "😴","ug": "😴",
    "ateist": "👦", "oduncu": "🪓", "fırıncı": "🥖", "beceriksiz": "🤕","bec": "🤕",
    "kütüphaneci": "📚", "bibliotecaria": "📚", "kurdumsu": "👱🌚", "wolfman": "👱🌚",
    "eros": "🏹", "fool": "🃏", "gof": "🃏&👳‍♀️", "köylü": "👱", "kemal": "👱",
    "kapıcı": "🏘", "bileyici": "👨🏻‍🦳", "afilador": "👨🏻‍🦳", "deli": "🤪",
    "hereje": "🦹‍♂️", "yabani çoçuk": "👶", "yç": "👶", "hain": "🖕",
    "lanetli": "😾", "kurtadam": "🐺", "kürt": "🐺", "alfa kurt": "⚡️","alfa kürt": "⚡️",
    "lycan": "🐺🌝", "yavru kurt": "🐶","yavru kürt": "🐶", "snow wolf": "🐺❄️",
    "kuduz kurt": "🐺🤢", "hızlı kurt": "🐺💨", "hızlı kürt": "🐺💨",
    "hungry wolf": "🐺🍖", "yaşlı kurt": "🐲", "falcı": "🔮", "taklitçi": "❌",
    "iblis": "👺", "survivor": "⛺️", "sk": "🔪", "seri katil": "🔪",
    "kundak": "🔥", "kundakçı": "🔥", "çg": "🎭", "çiftgiden": "🎭",
    "unutkan": "🤔", "tarikat": "👤", "tarikatçı": "👤", "guard": "🛡",
    "twin": "👯", "double agent": "👥"
}
...
async def startranked_takip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "startranked" in msg_text:
        ...
        await update.message.reply_text("✅ Yeni oyun başlatıldı, roller temizlendi!")
...
async def rol_ekle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Tüm girişi al ve ilk kelimeyi emoji kontrolü için seç
    full_input = " ".join(context.args).lower()
    first_word = context.args[0].lower()
    
    # Emojiyi ilk kelimeye göre seç, yoksa 👤 kullan
    emoji = ROLE_EMOJIS.get(first_word, "👤")
    
    if chat_id not in game_data:
        game_data[chat_id] = {"user_roles": {}, "last_msg_id": None}
    
    # Rolü kaydet (Kullanıcının yazdığı tam metni gösterir ama emojiyi ilk kelimeden seçer)
    game_data[chat_id]["user_roles"][user_id] = f"👤 {user_name}: {full_input.capitalize()} {emoji}"
    ...
    # MESAJ SİLME KALDIRILDI - Her zaman yeni bir liste mesajı atar
